...
REPORT_TYPE = [('html','HTML'),('csv','CSV'),('xls','XLS'),('rtf','RTF'),
               ('odt','ODT'),('ods','ODS'),('txt','Text'),('pdf','PDF'), 
               ('jrprint', 'Jasper Print')]

def CONVERT_TUPLE(text=[]):
    return str(tuple(text)).replace(',)', ')')

def _get_print_info(self, cr, uid, ids, data_form, context={}):
    sql = '''
        select now()::timestamp printdate, coalesce(partner.name, users.login) printedby 
        from %s wizard
        inner join res_users users on users.id = coalesce(wizard.write_uid, wizard.create_uid)
        left outer join res_partner partner on partner.id = users.partner_id
        where wizard.id = %s
    '''%(context.get('active_model').replace('.', '_'), context.get('active_id'))
    cr.execute(sql)
    return cr.fetchone()
