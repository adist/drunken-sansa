...
#        self.outputFormat = 'pdf'
    ...
    def execute(self):
        ...
        #Patch : Andri Adding capability of custom output
        elif self.data['output']:
            self.outputFormat = self.data['output']
        #End of Patch    
...
#        fd, dataFile = tempfile.mkstemp() # adista@bizoft 101312.1038 just add suffix so it called directly with associated program..
        fd, dataFile = tempfile.mkstemp(suffix=".csv")
        os.close(fd)
        fd, outputFile = tempfile.mkstemp(suffix=".csv")
        os.close(fd)
...
class report_jasper(report.interface.report_int):
    ...
    def create(self, cr, uid, ids, data, context):
        ...
        if self.parser:
            ...
            # adista@bizoft 041113.1746 add info how many times report been printed 
            # actually 'print_count' column can access directly from its query,, but it can't do such things: 
            #    1. update its counter automatically
            #    2. if jasper doesn't have query (data/record - using XPATH), it can't appear in its csv file
            # other report not yet implementing this way,, maybe someday it can be referenced.
            # 050613.1016 fix a way
            report_counter = pooler.get_pool(cr.dbname).get('bizoft.report.counter')
            if ids and report_counter:
                data['parameters']['print_count'] = \
                    str(report_counter.get_print_count_by_model(cr, uid, self.model, ids and ids[0] or False, context=context))
            #Patch Andri : Adding Custom output capability
            data['output'] = d.get('output','pdf')
#            data['output'] = d.get('output','jrprint')
            #End of Patch
